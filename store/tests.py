...
class ProductDestoyTestCase(APITestCase):
    def setUp(self):
        Product.objects.create(name='Simple product', description='Super product',
                               price=9.02)
...
class ProductListTestCase(APITestCase):
    def setUp(self):
        Product.objects.create(name='Simple product', description='Super product',
                               price=9.02)
        Product.objects.create(name='Other Product', description='Normal product',
                               price=9.02)
        Product.objects.create(name='Test product', description='Test product',
                               price=9.02)
...
class ProductUpdateTestCase(APITestCase):
    ...
    def test_update_product(self):
        ...
        response = self.client.patch(f'/api/v1/products/{product.id}/', product_attrs, format='json')
        updated = Product.objects.get(id=product.id)
        self.assertEqual(updated.name, product_attrs['name'])
        self.assertEqual(updated.description, product_attrs['description'])
        self.assertEqual(updated.price, product_attrs['price'])

    def test_upload_photo(self):
        product = Product.objects.first()
        original_photo = product.photo
        photo_path = os.path.join(settings.MEDIA_ROOT, 'products', 'vitamin-a.jpg',)
        ...
        try:
            updated = Product.objects.get(id=product.id)
            expected_photo = os.path.join(settings.MEDIA_ROOT, 'products', 'vitamin-a',)
            self.assertTrue(
